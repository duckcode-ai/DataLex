...
For new dbt adoption work, DataLex writes a domain-first workspace under the
repo's `DataLex/` folder:

```text
DataLex/
  datalex.yaml
  domains/
    commerce.yaml                         # kind: domain
  commerce/
    conceptual/order_revenue.diagram.yaml # kind: diagram, layer: conceptual
    logical/order_revenue.diagram.yaml    # kind: diagram, layer: logical
    physical/order_revenue.diagram.yaml   # kind: diagram, layer: physical
    contracts/order_revenue.contract.yaml # kind: contract
    proposals/order_revenue.proposal.yaml # kind: proposal
    glossary/revenue.term.yaml            # kind: term
    semantic/revenue.metric.yaml          # kind: metric_contract
  imported/dbt/commerce/orders.yaml       # imported dbt metadata overlays
  generated/dbt/commerce/orders.yml       # suggested dbt contract YAML
  generated-sql/ddl/
  generated-sql/migrations/
  Skills/
```

The loader also reads older layouts for compatibility:
...
include the domain-first tree plus the legacy paths above.
...
# Optional globs can be strings or lists. New workspaces use the
# domain-first defaults generated by the UI:
models:
  - "*/conceptual/**/*.yaml"
  - "*/logical/**/*.yaml"
  - "*/physical/**/*.yaml"
...
file by hand. New UI actions write domain-first locations such as
`commerce/conceptual/order_revenue.diagram.yaml`. Legacy
`datalex/diagrams/<slug>.diagram.yaml` files are still discoverable via the
`diagrams:` glob in `datalex.yaml`.
...
## `kind: contract` — certified business contract

Contracts are versioned business promises that can be exported into
`datalex-manifest.json` for DQL and agents. Draft, reviewed, and rejected
contracts stay in the repo but are excluded from the manifest.

```yaml
kind: contract
id: commerce.OrderRevenue.order_revenue
name: order_revenue
domain: commerce
entity: OrderRevenue
version: 1
...
source:
  kind: dbt_model
  ref: model.jaffle_shop.fct_orders
evidence:
  source_models:
    - model.jaffle_shop.fct_orders
  columns: [order_id, customer_id, order_total]
  tests: [order_id:unique]
  confidence: 0.84
```

Schema: [`datalex_core/_schemas/datalex/contract.schema.json`](../packages/core_engine/src/datalex_core/_schemas/datalex/contract.schema.json).

## `kind: proposal` — AI-generated review item

AI generation writes proposals first. Users certify or reject them before any
artifact becomes trusted.
...
status: draft
domain: commerce
target: commerce certification pack
proposed_change:
  files:
...
When a core certification proposal is certified, DataLex writes the approved
pack files above. Proposal types such as `metric_contract`, `glossary`, and
`dbt_contract` write only that focused artifact family.

Schema: [`datalex_core/_schemas/datalex/proposal.schema.json`](../packages/core_engine/src/datalex_core/_schemas/datalex/proposal.schema.json).

## `kind: metric_contract` — certified metric meaning

Metric contracts capture semantic metric meaning independently from physical
model contracts.
...
Schema: [`datalex_core/_schemas/datalex/metric_contract.schema.json`](../packages/core_engine/src/datalex_core/_schemas/datalex/metric_contract.schema.json).
